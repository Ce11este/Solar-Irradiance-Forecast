...
from tensorflow.keras.layers import LSTM, Bidirectional, LSTM, Input, Conv1D, Multiply, Flatten, MaxPooling1D, concatenate, GlobalAveragePooling1D, Reshape
...
model.add(LSTM(units=32, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.01), input_shape=(12, 12)))
model.add(Activation('relu'))
model.add(LSTM(units=16, kernel_regularizer=tf.keras.regularizers.l2(0.01),return_sequences=False))
model.add(Dropout(0.5))  # 添加Dropout层，指定丢弃率
model.add(Dense(units=3))
model.add(Activation('relu'))
adam = Adam(learning_rate=0.001)

model.compile(optimizer=adam, loss='mse',metrics = [tf.keras.metrics.RootMeanSquaredError()])#metrics = [tf.keras.metrics.RootMeanSquaredError()
