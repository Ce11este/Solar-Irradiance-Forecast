...
# LSTM-CNN Model
input_layer = tf.keras.layers.Input(shape=(x_train_multi.shape[1], x_train_multi.shape[2]))
cnn_layer = Conv1D(64, 3, activation='relu')(input_layer)
cnn_layer = MaxPooling1D(pool_size=2)(cnn_layer)
cnn_layer = Flatten()(cnn_layer)
cnn_layer = Reshape((1, cnn_layer.shape[1]))(cnn_layer) 
lstm_layer = LSTM(50, return_sequences=False)(cnn_layer)
dense_layer = Dense(50, activation='relu')(lstm_layer)
output_layer = Dense(future_target)(dense_layer)  # 输出多个时间步的预测值
...
model_checkpoint_callback = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_filepath, save_best_only=True, monitor='val_loss', mode='min', verbose=1)
